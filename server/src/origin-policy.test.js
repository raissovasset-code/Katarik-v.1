...
test('uses safe application and development origins by default', () => {
  ...
  for (const origin of DEFAULT_ALLOWED_ORIGINS) assert.equal(origins.has(origin), true);
});

test('environment configuration replaces defaults and normalizes values', () => {
  const origins = parseAllowedOrigins(' HTTPS://EXAMPLE.COM/ , https://localhost ');
...
test('allows configured browser origins and rejects all other browser origins', () => {
  const isOriginAllowed = createOriginPolicy(parseAllowedOrigins('https://game.example'));
