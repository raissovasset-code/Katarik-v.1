  ...
  return {
    ...
    players: [
      { id: "bot", isBot: true, active: true, hand, pogonRank: "4" },
      {
        id: "human",
        active: true,
        hand: Array.from({ length: opponentCards }, (_, i) => ({
          id: `x${i}`,
        })),
      },
    ],
...
test("generates groups, straights and double straights without brute force", () => {
  ...
  const types = new Set(
    generateBotMoves(gameWith(hand), gameWith(hand).players[0]).map(
      (move) => move.combo.type,
    ),
  );
...
test("strategically passes instead of wasting a bomb when there is no threat", () => {
  ...
  assert.deepEqual(chooseBotAction(gameWith(hand, table), "bot"), {
    type: "pass",
  });
});
...
test("after capturing the table, protects the pogon rank until it can finish the hand", () => {
  const hand = [card("4S", "4"), card("7H", "7", "normal", "H")];
  const game = gameWith(hand);
  game.mode = "pogoni";
  game.pogonReadyPlayerId = "bot";
...
test("after capturing the table, a real finishing pogon takes priority over every other move", () => {
  const hand = [card("7S", "7"), card("7H", "7", "normal", "H")];
  const game = gameWith(hand);
  game.mode = "pogoni";
  game.pogonReadyPlayerId = "bot";
  ...
  const action = chooseBotAction(game, "bot");
  assert.deepEqual(new Set(action.cardIds), new Set(["7S", "7H"]));
});
...
test("plans a straight with one pogon card and keeps the duplicate for the finishing pogon", () => {
  ...
  assert.deepEqual(new Set(lead.cardIds), new Set(["5S", "6S", "7S", "8S"]));

  game.players[0].hand = hand.filter(
    (cardItem) => !lead.cardIds.includes(cardItem.id),
  );
  game.table = null;
  game.pogonReadyPlayerId = "bot";
