...
test("neural training raises the chosen action score", () => {
  ...
  for (let index = 0; index < 100; index += 1)
    trainNeuralChoice(model, inputs, 0, 0.03);
...
test("positive and negative rewards move a sampled action in opposite directions", () => {
  ...
  const rewarded = createNeuralModel({
    inputSize: 3,
    hiddenSize: 6,
  ...
  const initial =
    neuralScore(rewarded, inputs[0]) - neuralScore(rewarded, inputs[1]);
  ...
  assert.ok(
    neuralScore(punished, inputs[0]) - neuralScore(punished, inputs[1]) <
      initial,
  );
});
...
test("sampled neural action is reproducible with seeded exploration", () => {
  ...
  const first = sampleNeuralAction(game, playerId, model, {
    random: createSeededRandom(10),
  });
  const second = sampleNeuralAction(game, playerId, model, {
    random: createSeededRandom(10),
  });
...
test("checkpoint replacement requires a completed and measurable improvement", () => {
  ...
  assert.equal(
    shouldReplaceNeuralCheckpoint(current, { winRate: 0.53, incomplete: 0 }),
    true,
  );
  assert.equal(
    shouldReplaceNeuralCheckpoint(current, { winRate: 0.522, incomplete: 0 }),
    false,
  );
  assert.equal(
    shouldReplaceNeuralCheckpoint(current, { winRate: 0.6, incomplete: 1 }),
    false,
  );
});
...
test("intermediate reward encourages shedding cards and discourages avoidable passes", () => {
  ...
  const playerId = game.currentPlayerId;
  const play = legalNeuralActions(game, playerId).find(
