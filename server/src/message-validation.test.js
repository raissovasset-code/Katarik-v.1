...
import { parseClientMessage, validateClientMessage } from './message-validation.js';
...
test('rejects missing and unknown message types', () => {
  ...
  assert.throws(() => validateClientMessage({ type: 'deleteEverything' }), /Неизвестный тип/);
});

test('validates create-room identity and mode', () => {
  assert.throws(
    () => validateClientMessage({ type: 'createRoom', mode: 'classic', ...identity, playerId: '' }),
    /Идентификатор игрока/,
  );
  assert.throws(
    () => validateClientMessage({ type: 'createRoom', mode: 'invalid', ...identity }),
    /Неизвестный режим/,
  );
  assert.throws(
    () => validateClientMessage({ type: 'createRoom', mode: 'classic', ...identity, name: 123 }),
    /Имя игрока/,
  );
});
...
test('validates join-room identity and six-character room code', () => {
  ...
  assert.throws(
    () => validateClientMessage({ type: 'joinRoom', roomId: 'ABC-12', ...identity }),
    /Некорректный код/,
  );
  assert.throws(
    () => validateClientMessage({ type: 'joinRoom', roomId: 'ABC123', ...identity, sessionToken: '' }),
    /Ключ восстановления/,
  );
});
...
test('validates played card ids and declared ranks', () => {
  assert.throws(() => validateClientMessage({ type: 'play', cardIds: [] }), /непустой список/);
  assert.throws(
    () => validateClientMessage({ type: 'play', cardIds: ['4S', '4S'] }),
    /повторения/,
  );
  assert.throws(
    () => validateClientMessage({ type: 'play', cardIds: ['DVK'], declaredRanks: [] }),
    /должны быть объектом/,
  );
  assert.throws(
    () => validateClientMessage({ type: 'play', cardIds: ['DVK'], declaredRanks: { DVK: 'JOKER' } }),
