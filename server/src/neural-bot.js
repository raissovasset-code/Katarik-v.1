...
export function legalNeuralActions(game, playerId) {
  ...
  if (player.hand.length === 1 && player.hand[0]?.type === "wild") {
    return [
      { type: "play", cardIds: [], declaredRanks: {}, combo: null, cards: [] },
    ];
  }
  const moves = generateBotMoves(game, player).map((move) => ({
    type: "play",
    ...move,
  }));
  if (game.table)
...
export function encodeStateAction(game, playerId, action) {
  ...
  const opponents = game.players.filter(
    (item) => item.id !== playerId && item.active,
  );
  const counts = Object.fromEntries(RANKS.map((rank) => [rank, 0]));
  for (const card of player.hand)
    if (card.type === "normal") counts[card.rank] += 1;
  ...
  return [
    ...
    opponentHands.reduce((sum, value) => sum + value, 0) /
      Math.max(opponentHands.length, 1) /
      28,
    opponents.length / 10,
...
export const NEURAL_INPUT_SIZE = encodeStateAction(
  {
    ...
    players: [
      { id: "x", active: true, hand: [], pogonRank: "4" },
      { id: "y", active: true, hand: [] },
    ],
  },
  "x",
  { type: "pass", cardIds: [], cards: [], combo: null },
).length;
...
export function loadPublishedNeuralModel() {
  try {
    const model = JSON.parse(
      fs.readFileSync(
        new URL("./neural-bot-model.json", import.meta.url),
        "utf8",
      ),
    );
    if (model.inputSize !== NEURAL_INPUT_SIZE || !model.metadata?.accepted)
      return null;
    return model;
  } catch {
    return null;
...
export function createNeuralModel({
  ...
  return {
    ...
    weights2: Array.from(
      { length: hiddenSize },
      () => (random() * 2 - 1) / Math.sqrt(hiddenSize),
    ),
    bias2: 0,
  };
}
...
export function neuralScore(model, input) {
  if (input.length !== model.inputSize)
    throw new Error("Neural input size mismatch");
  const hidden = model.weights1.map((row, index) =>
    Math.max(
      0,
      row.reduce(
  ...
  return hidden.reduce(
    (sum, value, index) => sum + value * model.weights2[index],
    model.bias2,
  );
}
...
export function trainNeuralChoice(
  ...
  caches.forEach((cache, actionIndex) => {
    ...
    cache.hidden.forEach((value, hiddenIndex) => {
      ...
      const hiddenGradient =
        outputGradient *
        model.weights2[hiddenIndex] *
        Number(cache.pre[hiddenIndex] > 0);
  ...
  model.weights2.forEach((_, index) => {
    model.weights2[index] -= learningRate * gradW2[index];
  });
  model.bias1.forEach((_, hiddenIndex) => {
    ...
    model.weights1[hiddenIndex].forEach((__, inputIndex) => {
      model.weights1[hiddenIndex][inputIndex] -=
        learningRate * gradW1[hiddenIndex][inputIndex];
    });
  });

  const predicted = probabilities.indexOf(Math.max(...probabilities));
  return {
    loss: -Math.log(Math.max(probabilities[chosenIndex], 1e-12)),
    correct: predicted === chosenIndex,
  };
}
...
export function sampleNeuralAction(
  ...
  const inputs = actions.map((action) =>
    encodeStateAction(game, playerId, action),
  );
  const scores = inputs.map(
    (input) => neuralScore(model, input) / Math.max(temperature, 0.05),
  );
...
export function neuralTrainingExample(game, playerId, teacherAction) {
  const actions = legalNeuralActions(game, playerId);
  const chosenIndex = actions.findIndex((action) =>
    sameAction(action, teacherAction),
  );
...
export function chooseNeuralAction(game, playerId, model) {
  ...
  return actions
    .map((action) => ({
      action,
      score: neuralScore(model, encodeStateAction(game, playerId, action)),
    }))
...
export function neuralStepReward(game, playerId, action) {
  ...
  if (action.type === "pass") {
    const hasPlayableMove = legalNeuralActions(game, playerId).some(
      (item) => item.type === "play",
    );
...
export function shouldReplaceNeuralCheckpoint(
  current,
  candidate,
  minimumGain = 0.005,
) {
