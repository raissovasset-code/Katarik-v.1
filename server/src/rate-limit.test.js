...
test('keeps create, join and command limits independent', () => {
  const limiter = new SlidingWindowRateLimiter();
  const context = { clientIp: '203.0.113.10', connectionId: 'socket-1', now: 100 };
  ...
  assert.doesNotThrow(() => enforceMessageRateLimit(limiter, { type: 'joinRoom' }, context));
  assert.doesNotThrow(() => enforceMessageRateLimit(limiter, { type: 'pass' }, context));
});
