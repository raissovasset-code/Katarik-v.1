...
function validatePlay(message) {
  ...
  if (message.cardIds.length > 55)
    throw new Error("В одном ходе слишком много карт");
  ...
  for (const [cardId, rank] of Object.entries(message.declaredRanks)) {
    ...
    if (!DECLARED_RANKS.has(rank))
      throw new Error("Объявлено недопустимое значение карты");
  }
}
...
export function validateClientMessage(message) {
  ...
  if (message.type === "createRoom") {
    ...
    if (!GAME_MODES.has(message.mode))
  ...
  if (message.type === "joinRoom") {
    ...
    if (!/^[A-Za-z0-9]{6}$/.test(message.roomId))
