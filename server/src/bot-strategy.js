...
function loadTrainedWeights() {
  try {
    const saved = JSON.parse(
      fs.readFileSync(
        new URL("./trained-bot-weights.json", import.meta.url),
        "utf8",
      ),
    );
...
export function generateBotMoves(game, player) {
  ...
  function add(cards, declaredRanks = {}) {
    ...
    if (
      candidates.has(key) ||
      leavesInvalidDvkRemainder(player.hand, new Set(ids))
    )
      return;
  ...
  for (let pairCount = 3; pairCount <= STRAIGHT_RANKS.length; pairCount += 1) {
    for (
      let start = 0;
      start <= STRAIGHT_RANKS.length - pairCount;
      start += 1
    ) {
      const ranks = STRAIGHT_RANKS.slice(start, start + pairCount);
      const groups = ranks.map((rank) =>
        combinations(byRank.get(rank) || [], 2),
      );
      if (groups.every((group) => group.length > 0)) {
        for (const cards of cartesian(groups)) add(cards);
      }
      if (dvk) {
        const shortRanks = ranks.filter(
          (rank) => (byRank.get(rank) || []).length === 1,
        );
        const missingRanks = ranks.filter(
          (rank) => (byRank.get(rank) || []).length === 0,
        );
...
function remainingHandScore(cards, weights) {
  ...
  return (
    grouped +
    cards.length * weights.remainingCard +
    specials * weights.remainingSpecial
  );
}
...
function isPogonTail(cards, rank) {
  if (
    !cards.length ||
    cards.length > 4 ||
    cards.some((card) => card.type === "joker")
  )
    return false;
  const normalCards = cards.filter((card) => card.type === "normal");
  if (!normalCards.length || !normalCards.every((card) => card.rank === rank))
    return false;
  const declaredRanks = cards.some((card) => card.type === "wild")
    ? { DVK: rank }
    : {};
...
function endgameCaptureGroup(game, player, moves) {
  ...
  const captureCards = player.hand.filter(
    (card) =>
      card.rank !== "RED_JOKER" &&
  ...
  if (
    redJokers.length !== 1 ||
    !pogonCards.length ||
    ![3, 4].includes(captureCards.length)
  )
    return null;
  if (
    !captureCards.every(
      (card) => card.type === "normal" && card.rank === captureCards[0].rank,
    )
  ) {
    return null;
  }
  return (
    moves.find(
      (move) =>
        ...
        move.cardIds.every((id) =>
          captureCards.some((card) => card.id === id),
        ) &&
...
function preferredFreeLead(game, player, moves) {
  ...
  const narrowCombinations = moves.filter((move) =>
    ["straight", "doubleStraight"].includes(move.combo.type),
  );
  if (narrowCombinations.length) {
    return narrowCombinations.sort(
      (left, right) =>
        right.cardIds.length - left.cardIds.length ||
        left.combo.high - right.combo.high ||
        Number(right.combo.type === "doubleStraight") -
          Number(left.combo.type === "doubleStraight"),
    );
  }

  let singlesAndPairs = moves.filter((move) =>
    ["single", "pair"].includes(move.combo.type),
  );
  const rankCounts = cardsByRank(player.hand);
  const withoutBrokenCaptureGroups = singlesAndPairs.filter(
    (move) =>
      !move.cards.some(
        (card) =>
          card.type === "normal" &&
          (rankCounts.get(card.rank)?.length || 0) >= 3,
      ),
  );
  if (withoutBrokenCaptureGroups.length)
    singlesAndPairs = withoutBrokenCaptureGroups;
  const withoutDvk = singlesAndPairs.filter(
    (move) => !move.cards.some((card) => card.type === "wild"),
  );
  if (withoutDvk.length) singlesAndPairs = withoutDvk;
  if (singlesAndPairs.length) {
    return singlesAndPairs.sort(
      (left, right) =>
        left.combo.high - right.combo.high ||
        Number(right.combo.type === "pair") -
          Number(left.combo.type === "pair"),
    );
  }
...
function preferredTableResponse(game, player, moves) {
  const pogonSetupMoves = moves.filter((move) =>
    setsUpPogon(game, player, move),
  );
  if (pogonSetupMoves.length) return pogonSetupMoves;

  const sortWeakestFirst = (candidates) =>
    candidates.sort(
      (left, right) =>
  ...
  const sameType = moves.filter(
    (move) => move.combo.type === game.table.combo.type,
  );
  const rankCounts = cardsByRank(player.hand);
  const sameTypeWithoutBrokenGroups = sameType.filter(
    (move) =>
      !move.cards.some((card) => {
        if (card.type !== "normal") return false;
        const cardsOfRankInMove = move.cards.filter(
          (selected) =>
            selected.type === "normal" && selected.rank === card.rank,
        ).length;
        return (
          (rankCounts.get(card.rank)?.length || 0) >= 3 &&
  ...
  if (sameTypeWithoutBrokenGroups.length)
    return sortWeakestFirst(sameTypeWithoutBrokenGroups);
...
function moveScore(game, player, move, weights) {
  ...
  const activeOpponents = game.players.filter(
    (item) => item.id !== player.id && item.active,
  );
  const lowestOpponentHand = Math.min(
    ...activeOpponents.map((item) => item.hand.length),
    99,
  );
  ...
  let score =
    move.cardIds.length * (urgent ? weights.cardsUrgent : weights.cardsNormal);
  score += remainingHandScore(remaining, weights);
  score +=
    move.combo.high *
    (game.table ? weights.activeTableHigh : weights.freeTableHigh);
  ...
  if (move.combo.type === "quad" && !urgent && !winsNow)
    score += weights.preserveQuad;
  if (usesDvk && !urgent && !winsNow) score += weights.preserveDvk;
  if (usesJoker && !urgent && !winsNow) {
    score += game.table
      ? weights.preserveJokerActive
      : weights.preserveJokerFree;
  }
  if (urgent)
    score +=
      move.combo.high * weights.urgentHigh +
      move.cardIds.length * weights.urgentCard;
  ...
  if (isPogoni && canSetPogon && winsNow) {
    ...
    if (
      counted.length &&
      counted.every((card) => card.rank === player.pogonRank)
    )
      score += 30_000;
  }
...
export function chooseBotAction(game, playerId, weights = TRAINED_BOT_WEIGHTS) {
  ...
  if (!player || game.currentPlayerId !== playerId || game.status !== "playing")
    return null;
  ...
  if (game.mode === "pogoni" && game.pogonReadyPlayerId !== player.id) {
    ...
    if (movesPreservingPogon.length) {
      ...
      const nonFinishingMoves = moves.filter(
        (move) => move.cardIds.length < player.hand.length,
      );
      if (nonFinishingMoves.length) moves = nonFinishingMoves;
    }
  } else if (game.mode === "pogoni" && game.pogonReadyPlayerId === player.id) {
    const finishingPogonMoves = moves.filter((move) =>
      finishesPogon(game, player, move),
    );
    if (finishingPogonMoves.length) {
      moves = finishingPogonMoves;
    } else {
      const movesWithoutPogon = moves.filter(
        (move) =>
          !move.cards.some(
            (card) => card.type === "normal" && card.rank === player.pogonRank,
          ),
  ...
  const lowestOpponentHand = Math.min(
    ...game.players
      .filter((item) => item.id !== player.id && item.active)
  ...
  if (game.table && !urgent && player.hand.length > 4) {
    if (
      !preparesPogon &&
      best.combo.type === "quad" &&
      game.table.combo.type !== "quad"
    ) {
      return { type: "pass" };
    }
    if (!preparesPogon && usesDvk && best.cardIds.length <= 2)
      return { type: "pass" };
  }
