...
const candidatePath = path.resolve(
  argument("output", path.join(__dirname, "../src/neural-bot-candidate.json")),
);
  ...
  for (let gameIndex = 0; gameIndex < imitationGames; gameIndex += 1) {
    simulateBotGame({
      ...
      onDecision: ({ game, player, action }) => {
        ...
        const result = trainNeuralChoice(
          model,
          example.inputs,
          example.chosenIndex,
          0.008,
...
const neuralPolicy = (game, playerId) =>
  chooseNeuralAction(game, playerId, model);
...
console.log(
  JSON.stringify({
    candidatePath,
    publishedPath: accepted ? publishedPath : null,
    accepted,
    tournament,
  }),
);
